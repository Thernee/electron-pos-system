...
export class ReportsService {
  ...
  // Customer balances: cash + wallet - debt
  async customerBalances() {
    ...
    return customers.map(c => ({
      id: c.id,
      name: c.name,
      balance: c.cashBalance + c.walletBalance - c.debtBalance,
      debt: c.debtBalance,
    }));
  }
  ...
  async adminSummary() {
    const sumBalances = await this.prisma.customer.aggregate({
      _sum: { cashBalance: true, walletBalance: true, debtBalance: true },
    });
    ...
    const totalCustomer =
      ...
      (sumBalances._sum.walletBalance || 0) -
      (sumBalances._sum.debtBalance || 0);
    return {
      totalCustomer,
